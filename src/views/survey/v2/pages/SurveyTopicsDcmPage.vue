...
import {
    ...
    SurveyServerTopicDcmConfigData,
...
function getDcmTitle(): string {
    return configData().sections[0].answers_group?.topics_dcm?.topic as string;
}

function getChoices(): DcmAnswer[] {
    const res: DcmAnswer[] = [];

    const dcm = configData().sections[0].answers_group?.topics_dcm as SurveyServerTopicDcmConfigData;
    for (let i = 0; i < dcm.choices.length; ++i) {
        for (let j = 0; j < dcm.choices[i].items.length; ++j) {
            if (j !== 0) {
                dcm.choices[i].items[j] = dcm.choices[i].items[j].toLowerCase()
            }
        }
        res.push({
            id: dcm.choices[i].id,
            text: dcm.choices[i].items.join(" ")
        });
    }
...
<template>
    ...
    <div class="gb-ui-section-bg-survey gb-layout-tl w-fit relative">
        <div class="dcm-page-counter">{{ configData().sections[0].answers_group?.topics_dcm?.index}} of 5</div>
        <div class="gb-layout-t-between relative pt-2">
            <div>
                <span class="font-bold" v-html="getDcmTitle()"></span>
                <span class="pl-2 text-[0.7em]">{{translator().translate("SV_213__")}}</span>
            </div>
        </div>
        <div class="pt-4 pb-4">
